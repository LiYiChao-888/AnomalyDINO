...
def get_test_gt_map(object_name, anomaly_type, img_nr, experiment, data_root, dataset, good=False):
    """
    ...
    if not good:
        # 尝试适配两种命名风格
        gt_base_lower = f"{data_root}/{object_name}/ground_truth/{anomaly_type}/{img_nr}"
        gt_base_camel = f"{data_root}/{object_name}/GroundTruth/{anomaly_type}/{img_nr}"
        
        suffix = "_mask.png" if dataset == "MVTec" or dataset == "ydfid" else ".png"
        
        # 优先检测 CamelCase (GroundTruth)
        if os.path.exists(gt_base_camel + suffix):
            gt_path = gt_base_camel + suffix
        else:
            gt_path = gt_base_lower + suffix

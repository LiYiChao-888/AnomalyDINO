...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir, dataset):
    ...
    for subdir in listdir(str(test_dir)):
        ...
        test_images = [path.splitext(file)[0]
                       ...
                       if path.splitext(file)[1] == ('.png' if dataset == "MVTec" or dataset == "ydfid" else '.JPG')]

        # If subdir is not 'good', derive corresponding GT names.
        if subdir != 'good':
            gt_filenames.extend(
                [path.join(gt_base_dir, subdir, file + '_mask.png' if dataset == "MVTec" or dataset == "ydfid" else file + '.png')
...
def get_objects_from_dataset(dataset, data_path=None):
    if dataset == "MVTec":
        ...
        if data_path is None or not os.path.exists(data_path):
             raise ValueError(f"自定义数据集 '{dataset}' 需要提供有效的 data_path 当前路径: {data_path}")
        
        objects = []
        # 尝试使用 natsorted 进行自然排序，如果没有安装则回退到普通 sorted
        try:
            from natsort import natsorted
            dir_list = natsorted(os.listdir(data_path))
        except ImportError:
...
def eval_finished_run(dataset, dataset_base_dir, anomaly_maps_dir, output_dir, seed = None, pro_integration_limit = 0.3, eval_clf = True, eval_segm = False, delete_tiff_files = True, aggregation_statistics = "meantop1p"):
    ...
    objects = get_objects_from_dataset(dataset, data_path=dataset_base_dir)

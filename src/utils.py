...
def get_dataset_info(dataset, preprocess, data_path=None):
    ...
    if dataset == "MVTec":
        ...
        if data_path is None:
            raise ValueError("使用 ydfid 数据集时请必须提供 'data_path' 参数！")

        objects = []
        object_anomalies = {}
        
        # 2. 自动检索所有物体文件夹 (CL1~..., SL1~..., SP1~...)
        # 使用 sorted 确保顺序一致
        if not os.path.exists(data_path):
             raise ValueError(f"数据路径不存在: {data_path}")

        #for o in sorted(os.listdir(data_path)):
        for o in sorted(os.listdir(data_path), key=natural_key):
            o_path = os.path.join(data_path, o)
            ...
            if os.path.isdir(o_path):
                ...
                # 3. 自动检索异常类型
                # 你的结构是 test/bad 和 test/good
                test_dir = os.path.join(o_path, "test")
                anomaly_types = []
                
                if os.path.exists(test_dir):
                    for subdir in os.listdir(test_dir):
                        subdir_path = os.path.join(test_dir, subdir)
                        # 【关键】排除 'good'，剩下的文件夹即为异常类型
                        if os.path.isdir(subdir_path) and subdir != "good":
                            anomaly_types.append(subdir)
                
                object_anomalies[o] = anomaly_types

        print(f"已自动检索到 {len(objects)} 个物体: {objects}")

        # 4. 配置默认的 Masking 和 Rotation
        if preprocess in ["informed_no_mask", "agnostic_no_mask"]:
            masking_default = {o: False for o in objects}
        else:
            masking_default = {o: True for o in objects}
